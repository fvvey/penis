...
def print_results(results: list, time_taken: float):
    ...
    print(f"THE TIME THIS SHIT SHOULD TAKE: {ratelimit_multi}\n"
          f"TIME THIS STUPID SHIT TOOK (PROBABLY SLOW AS HELL NO CAP): {round(time_taken, 4)}s")
...
def snipe_item(data, proxy=None, sleep_time=-1):
    ...
    if debugging:
        results.append(f'THE CURRENT PRICE IS {price} BE PATIENT NIGGA')
    ...
    print("FOUND A BITCH DATS CHEAPER THAN YO MAX PRICE")
    ...
    if check.ok:
        try:
            print("COPPED THIS HO WITH A RESPONSE OF 200 (THAT MEANS DIS SHIT ALL GOOD)\n\n"
                    f"Information: \n"
            ...
            perm_results.append(
                    "COPPED THIS HO WITH A RESPONSE OF 200 (THAT MEANS DIS SHIT ALL GOOD)\n\n"
                    "Information: \n"
                    f"Purchased: {check.json()['purchased']}\n"
                    f"Asset bought: {check.json()['assetName']}\n"
                    f"Asset type: {check.json()['assetType']}\n"
            ...
            with open("buy logs.txt", "a") as f:
                f.write(f"\n\n\nGot a respone code of: {check.status_code}. Reason: {check.reason}\n\n"
                        ...
                        f"Bought for: {price}\n\n\n")
            if config["webhook"] != "":
                r.post(config["webhook"], data={'content': f"{'@everyone' if config['pingall'] else ''} | YO FVVEY MY NIGGA I JUST COPPED "
                                                        f"https://www.roblox.com/catalog/{data['asset']} FOR **{price} ROBUX** FO SHIZZLE MY NIZZLE"})
            ...
            if config["webhook"] != "":
                r.post(config["webhook"], data={
                    'content': f"{'@everyone' if config['pingall'] else ''} | ROBLOX A FUCKING OPP CUZ SOME DUMBASS SHIT WENT WRONG WHILE TRYNA COP "
                                f" https://www.roblox.com/catalog/{data['asset']} FO **{price} ROBUX** BUILDERMAN A HO ON GOD. "
                                f"CHECK LOGS FO MO INFO FOO"})
        ...
        with open("buy logs.txt", "a") as f:
            f.write(f"\n\n\nSomething went wrong whilst buying the item.\nGot a respone code of: {check.status_code}."
                    ...
                    f"{check.headers}\n\n\n")
        if config["webhook"] != "":
            r.post(config["webhook"], data={'content': f"{'@everyone' if config['pingall'] else ''} | ROBLOX A FUCKING OPP CUZ SOME DUMBASS SHIT WENT WRONG WHILE TRYNA COP "
                                                    f"https://www.roblox.com/catalog/{data['asset']} FO **{price} ROBUX** BUILDERMAN A HO ON GOD. CHECK LOGS FO MO INFO FOO"})
